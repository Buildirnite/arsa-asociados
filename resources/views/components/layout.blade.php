...
<body class="min-h-screen flex flex-col bg-white text-midnight-900 font-sans antialiased">
    ...
    @php
        ...
        $waSvg = 'M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z';
        $waConsultas = [
            ['Derecho de Familia', '👨‍👩‍👧', 'Hola, necesito asesoría en Derecho de Familia (divorcio, pensión de alimentos o cuidado personal).'],
            ['Problema laboral',    '💼', 'Hola, tengo una consulta laboral (despido, finiquito o cobro de cotizaciones).'],
            ['Civil o contratos',   '📄', 'Hola, necesito ayuda con un asunto civil o la revisión de un contrato.'],
            ['Agendar una consulta','📅', 'Hola, me gustaría agendar una primera consulta con Arsa & Asociados.'],
        ];
    @endphp
    <div class="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        {{-- Panel de chat --}}
        <div id="wa-panel"
             class="hidden origin-bottom-right scale-95 opacity-0 transition-all duration-200 w-[calc(100vw-3rem)] max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
            {{-- Cabecera --}}
            <div class="bg-[#075E54] px-4 py-3 flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-white/15 flex items-center justify-center shrink-0">
                    <img src="{{ asset('images/brand/logo-icon.png') }}" alt="Arsa & Asociados" class="h-7 w-auto object-contain">
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-white text-sm font-semibold leading-tight">Arsa &amp; Asociados</p>
                    <p class="text-white/70 text-xs flex items-center gap-1.5">
                        <span class="inline-block w-2 h-2 rounded-full bg-green-400"></span>
                        Normalmente responde en minutos
                    </p>
                </div>
                <button id="wa-close" aria-label="Cerrar" class="text-white/80 hover:text-white transition-colors p-1">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </div>
            {{-- Cuerpo estilo chat --}}
            <div class="px-4 py-5 bg-[#ECE5DD] max-h-[60vh] overflow-y-auto">
                <div class="bg-white rounded-lg rounded-tl-none shadow-sm px-4 py-3 mb-4 max-w-[85%]">
                    <p class="text-sm text-midnight-700 leading-relaxed">
                        👋 ¡Hola! Bienvenido a <strong>Arsa &amp; Asociados</strong>. ¿Sobre qué tema necesitas asesoría? Toca una opción y te respondemos al instante.
                    </p>
                </div>
                <div class="space-y-2">
                    @foreach($waConsultas as [$label, $emoji, $text])
                        <a href="https://wa.me/{{ $waNumber }}?text={{ rawurlencode($text) }}"
                           target="_blank" rel="noopener noreferrer"
                           class="flex items-center gap-3 w-full bg-white rounded-lg px-4 py-3 text-left shadow-sm hover:bg-gold-50 hover:shadow transition-all group">
                            <span class="text-lg shrink-0">{{ $emoji }}</span>
                            <span class="text-sm font-medium text-midnight-800 flex-1">{{ $label }}</span>
                            <svg class="w-4 h-4 text-midnight-300 group-hover:text-gold-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        {{-- Teaser (burbuja que aparece sola) --}}
        <div id="wa-teaser"
             class="hidden items-center gap-2 bg-white rounded-full shadow-lg pl-4 pr-2 py-2 max-w-[15rem]">
            <span class="text-sm text-midnight-700">👋 ¿Tienes una consulta legal?</span>
            <button id="wa-teaser-close" aria-label="Cerrar" class="text-midnight-300 hover:text-midnight-600 shrink-0">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>

        {{-- Botón --}}
        <button id="wa-toggle" type="button" aria-label="Abrir chat de WhatsApp"
                class="relative w-14 h-14 rounded-full bg-[#25D366] flex items-center justify-center shadow-lg hover:bg-[#1da851] transition-colors self-end">
            <span class="absolute inset-0 rounded-full bg-[#25D366] opacity-60 animate-ping" id="wa-ping"></span>
            <span id="wa-badge" class="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-red-500 text-white text-[11px] font-bold flex items-center justify-center shadow animate-bounce">1</span>
            <svg class="w-7 h-7 text-white relative" fill="currentColor" viewBox="0 0 24 24"><path d="{{ $waSvg }}"/></svg>
        </button>
    </div>
    ...
    <script>
        (function () {
            ...
            const teaserClose = document.getElementById('wa-teaser-close');
            let open = false;
            ...
            teaser.addEventListener('click', (e) => { if (e.target !== teaserClose && !teaserClose.contains(e.target)) openPanel(); });
            teaserClose.addEventListener('click', (e) => { e.stopPropagation(); hideTeaser(); sessionStorage.setItem('wa_teaser_dismissed', '1'); });

            // Cerrar al hacer clic fuera del widget
            document.addEventListener('click', (e) => {
                if (open && !e.target.closest('#wa-panel') && !e.target.closest('#wa-toggle')) closePanel();
            });

            // Mostrar el teaser una vez por sesión, a los pocos segundos
            if (!sessionStorage.getItem('wa_teaser_dismissed')) {
                setTimeout(() => {
                    if (!open) { teaser.classList.remove('hidden'); teaser.classList.add('flex'); }
                }, 4000);
            }
        })();
    </script>
